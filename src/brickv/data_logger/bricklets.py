...
class BarometerBricklet(AbstractBricklet):
    ...
    def start_timer(self, data):        
        ...
        value3 = DataLogger.parse_to_int(data[BAROMETER_CHIP_TEMPERATURE])  
...
class Current12Bricklet(AbstractBricklet):
    ...
    def start_timer(self, data):
        ...
        value1 = DataLogger.parse_to_int(data[CURRENT_12_ANALOG_VALUE])
        value2 = DataLogger.parse_to_int(data[CURRENT_12_CURRENT])  
...
class Current25Bricklet(AbstractBricklet):
    ...
    def start_timer(self, data):
        ...
        value1 = DataLogger.parse_to_int(data[CURRENT_25_ANALOG_VALUE])
        value2 = DataLogger.parse_to_int(data[CURRENT_25_CURRENT])  
...
class DistanceIRBricklet(AbstractBricklet):
    ...
    def start_timer(self, data):
        ...
        value1 = DataLogger.parse_to_int(data[DISTANCE_IR_ANALOG_VALUE])
        value2 = DataLogger.parse_to_int(data[DISTANCE_IR_DISTANCE])  
...
class DistanceUSBricklet(AbstractBricklet):
    ...
    def start_timer(self, data):
        ...
        value1 = DataLogger.parse_to_int(data[DISTANCE_US_DISTANCE])
...
class DualButtonBricklet(AbstractBricklet):
    ...
    def start_timer(self, data):
        ...
        value1 = DataLogger.parse_to_int(data[DUAL_BUTTON_BUTTONS])
        value2 = DataLogger.parse_to_int(data[DUAL_BUTTON_LEDS])
    ...
    def _timer_buttons(self):
        try:
            ...
            DataLogger.Q.put(CSVData(self.uid, DUAL_BUTTON, DUAL_BUTTON_BUTTON_L, button_l))
            DataLogger.Q.put(CSVData(self.uid, DUAL_BUTTON, DUAL_BUTTON_BUTTON_R, button_r))
        except Exception as e:
            DataLogger.Q.put(CSVData(self.uid, DUAL_BUTTON, DUAL_BUTTON_BUTTONS, self._exception_msg(e.value, e.description)))

    def _timer_leds(self):
        try:
            led_l, led_r = self._device.get_led_state()
            DataLogger.Q.put(CSVData(self.uid, DUAL_BUTTON, DUAL_BUTTON_LED_L, led_l))
            DataLogger.Q.put(CSVData(self.uid, DUAL_BUTTON, DUAL_BUTTON_LED_R, led_r))
        except Exception as e:
            DataLogger.Q.put(CSVData(self.uid, DUAL_BUTTON, DUAL_BUTTON_LEDS, self._exception_msg(e.value, e.description)))
...
class DualRelayBricklet(AbstractBricklet):
    ...
    def start_timer(self, data):
        ...
        value1 = DataLogger.parse_to_int(data[DUAL_RELAY_STATE])
    ...
    def _timer_state(self):
        try:
            ...
            DataLogger.Q.put(CSVData(self.uid, DUAL_RELAY, DUAL_RELAY_1, r1))
            DataLogger.Q.put(CSVData(self.uid, DUAL_RELAY, DUAL_RELAY_2, r2))
        except Exception as e:
            DataLogger.Q.put(CSVData(self.uid, DUAL_RELAY, DUAL_RELAY_STATE, self._exception_msg(e.value, e.description)))
...
class GPSBricklet(AbstractBricklet):
    ...
    def start_timer(self, data):
        ...
        value1 = DataLogger.parse_to_int(data[GPS_COORDINATES])
        value2 = DataLogger.parse_to_int(data[GPS_ALTITUDE])
        value3 = DataLogger.parse_to_int(data[GPS_MOTION])
        value4 = DataLogger.parse_to_int(data[GPS_DATE_TIME])
    ...
    def _timer_coordinates(self):
        try:
            ...
            DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_FIX_STATUS, fix))
            DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_SATELLITES_VIEW, satellites_view))
            DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_SATELLITES_USED, satellites_used))
              
            if fix == GPS.FIX_NO_FIX:
                DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_COORDINATES, self._exception_msg("Fix-Status="+fix, "GPS Fix-Status was 1, but needs to be 2 or 3 for valid Coordinates.")))
                return
            ...
            DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_LATITUDE, latitude))
            DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_NS, ns))
            DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_LONGITUDE, longitude))
            DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_EW, ew))
            DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_PDOP, pdop))
            DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_HDOP, hdop))
            DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_VDOP, vdop))
            DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_EPE, epe))
        except Exception as e:
            DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_COORDINATES, self._exception_msg(e.value, e.description)))
    ...
    def _timer_altitude(self):
        try:
            ...
            if fix != GPS.FIX_3D_FIX:
                DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_ALTITUDE, self._exception_msg("Fix-Status="+fix, "GPS Fix-Status was " + fix + ", but needs to be 3 for valid Altitude Values.")))
                return

            altitude, geoidal_separation = self._device.get_altitude()            
            DataLogger.Q.put(CSVData(self.uid, GPS_ALTITUDE, GPS_ALTITUDE, altitude))
            DataLogger.Q.put(CSVData(self.uid, GPS_ALTITUDE, GPS_GEOIDAL_SEPERATION, geoidal_separation))
        except Exception as e:
            DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_ALTITUDE, self._exception_msg(e.value, e.description)))
    ...
    def _timer_motion(self):
        try:
            ...
            if fix == GPS.FIX_NO_FIX:
                DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_MOTION, self._exception_msg("Fix-Status="+fix, "GPS Fix-Status was " + fix + ", but needs to be 2 or 3 for valid Altitude Values.")))
                return

            course, speed = self._device.get_motion()
            DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_COURSE, course))
            DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_SPEED, speed))
        except Exception as e:
            DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_MOTION, self._exception_msg(e.value, e.description)))

    def _timer_date_time(self):
        try:
            date, time = self._device.get_date_time()
            DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_DATE, date))
            DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_TIME, time))
        except Exception as e:
            DataLogger.Q.put(CSVData(self.uid, GPS_BRICKLET, GPS_DATE_TIME, self._exception_msg(e.value, e.description)))
...
class HallEffectBricklet(AbstractBricklet):
    ...
    def start_timer(self, data):
        ...
        value1 = DataLogger.parse_to_int(data[HALL_EFFECT_VALUE])
...
class IndustrialDual020mABricklet(AbstractBricklet):
    ...
    def start_timer(self, data):
        ...
        value1 = DataLogger.parse_to_int(data[INDUSTRIAL_DUAL_0_20_MA_CURRENT])      
...
class IO16Bricklet(AbstractBricklet):
    ...
    def start_timer(self, data):
        ...
        value1 = DataLogger.parse_to_int(data[IO_16_PORTS])      
...
class IO4Bricklet(AbstractBricklet):
    ...
    def start_timer(self, data):
        ...
        value1 = DataLogger.parse_to_int(data[IO_4_VALUE])     
...
class JoystickBricklet(AbstractBricklet):
    ...
    def start_timer(self, data):
        ...
        value1 = DataLogger.parse_to_int(data[JOYSTICK_POSITION])     
        value2 = DataLogger.parse_to_int(data[JOYSTICK_ANALOG_VALUE])    
        value3 = DataLogger.parse_to_int(data[JOYSTICK_PRESSED])    
